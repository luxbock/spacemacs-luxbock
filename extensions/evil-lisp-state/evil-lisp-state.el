...
;; Inherit key-bindings from normal-state

(setq evil-lisp-state-map evil-normal-state-map)
...
(eval-and-compile
  ...
  (defcustom evil-lisp-state-major-modes '(emacs-lisp-mode clojure-mode)
...
If `evil-lisp-state-global' is non nil then this variable has no effect."
  ...
  (defcustom evil-lisp-state-left "[([{\"]"
    "Regexp that matches opening parentheses."
    :type 'string
    :group 'evil-lisp-state)

  (defcustom evil-lisp-state-right "[)]}\"]"
    "Regexp that matches closing parentheses."
    :type 'string
    :group 'evil-lisp-state)

  (defcustom evil-lisp-state-parens "[][(){}]+"
    "Regexp that matches all parentheses."
    :type 'string
    :group 'evil-lisp-state))
...
(defun evil-lisp-state-move-by-word (fn count)
  "Calls `fn' `count' times skipping the parentheses as defined
by `evil-lisp-state-parens'"
  (dotimes (c count)
    (funcall fn)
    (while (looking-at evil-lisp-state-parens)
      (funcall fn))))
...
(defun lisp-state-forward-word-begin (count)
  "Calls `evil-forward-word-begin' `count' amount of times,
skipping parentheses on the way"
  (interactive "p")
  (evil-lisp-state-move-by-word #'evil-forward-word-begin count)) 
...
(defun lisp-state-forward-word-end (count)
  "Calls `evil-forward-word-end' `count' amount of times,
skipping parentheses on the way"
  (interactive "p")
  (evil-lisp-state-move-by-word #'evil-forward-word-end count))
...
(defun lisp-state-backward-word-begin (count)
  "Calls `evil-backward-word-begin' `count' amount of times,
skipping parentheses on the way"
  (interactive "p")
  (evil-lisp-state-move-by-word #'evil-backward-word-begin count))
...
(defun lisp-state-backward-word-end (count)
  "Calls `evil-backward-word-end' `count' amount of times,
skipping parentheses on the way"
  (interactive "p")
  (evil-lisp-state-move-by-word #'evil-backward-word-end count))
...
(defconst evil-lisp-state-commands
  `(
    ...
    ;; ("9"   . digit-argument)
    ;; ("a"   . sp-absorb-sexp)
    ("b"   . lisp-state-backward-word-begin)
    ("B"   . lisp-state-backward-WORD-begin)
    ;; ("c"   . sp-convolute-sexp)
    ;; ("ds"  . sp-kill-symbol)
    ...
    ("e"   . lisp-state-forward-word-end)
    ("E"   . lisp-state-forward-WORD-end)
    ("ge"  . lisp-state-backward-word-end)
    ("gE"  . lisp-state-backward-WORD-end)
    ;; ("i"   . evil-insert-state)
    ;; ("I"   . evil-insert-line)
    ;; ("J"   . sp-join-sexp)
    ...
    ("w"   . lisp-state-forward-word-begin)
    ("W"   . lisp-state-forward-WORD-begin)
    ;; ("y"   . sp-copy-sexp)
    )
